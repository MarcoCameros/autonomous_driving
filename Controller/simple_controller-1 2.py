...
def hough_cv2(image,img_mask):
    ...
    if lines is not None and len(lines)<30:
        ...
        for line in lines:
            for x1, y1, x2, y2 in line:
                ...
                if x1 < 128 and x2 < 128:
                    ...
                    cv2.line(img_lines, (x1, y1), (x2, y2), (200, 0, 0), 4)
                elif x1 > 128 and x2 > 128:
                    right_lines.append((x1, y1, x2, y2))
                    cv2.line(img_lines, (x1, y1), (x2, y2), (0, 200, 0), 4)
        ...
        if left_lines and right_lines:
            ...
            cv2.line(img_lines, (mid_x1, mid_y1), (mid_x2, mid_y2), (0, 0, 200), 6)
...
def main():
    ...
    while robot.step() != -1:
        ...
        # Volver al modo automático si ya no hay entrada manual
        if manual_mode and (current_time - last_manual_input_time) > MANUAL_TIMEOUT:
            print("[INFO] Returning to auto mode")
            manual_mode = False

        # Solo ejecutar algoritmo automático si no estás en modo manual
        if not manual_mode:
            hough_image = hough_cv2(image, roi_image)
        else:
            hough_image = image  # mostrar imagen sin anotaciones automáticas
