...
print("Carpeta del script:", current_dir)


model_path = os.path.join(current_dir, 'modelo_nvidia.h5')
print("Carpeta del modelo:", model_path)

if os.path.exists(model_path):
    print(f"✅ El modelo existe en: {'modelo_nvidia.h5'}")
...
def preprocess_image(img_resized):
    img_normalized = img_resized.astype(np.float32) / 127.5 - 1.0
    input_tensor = np.expand_dims(img_normalized, axis=0)  # (1, 66, 200, 3)
    return input_tensor
...
def main():
    ...
    # Para registrar tiempo
    last_photo_time = -float('inf')

    while robot.step() != -1:
        # Get image from camera
        image = get_image(camera)[:, :, :3] 
        ...
        current_time = robot.getTime()  # Tiempo simulado
        ...
        PHOTO_INTERVAL = 1.0  # segundos

        # Dentro de tu bucle principal
        current_time = robot.getTime()  # Tiempo simulado en segundos
        ...
        if current_time - last_photo_time >= PHOTO_INTERVAL:
            ...
            steering_angle = model.predict(process_image)[0][0]
            print("Ángulo de dirección predicho:", steering_angle)
